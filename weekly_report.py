...
with col4:
    st.metric(label='Quotes % Change', value=f"{round(df['change_quotes'].head(slider_val), 2)}%")
    
with col5:
    st.metric(label='NB Apps % Change', value=f"{round(df['change_nb_apps'].head(slider_val), 2)}%")
    
with col6:
    st.metric(label='RW Apps % Change', value=f"{round(df['change_rw_apps'].head(slider_val), 2)}%")
st.write('---')
