...
with col4:
    st.metric(label='AVG WOW Quotes % Change WOW', value=f"{round(df['pct_change_quotes'].head(slider_val).mean(), 2)}%")
    
with col5:
    st.metric(label='AVG WOW NB Apps % Change WOW', value=f"{round(df['pct_change_nb_apps'].head(slider_val).mean(), 2)}%")
